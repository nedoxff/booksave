...
const MEDIA_ENDPOINT: string = "https://x.com/i/api/graphql/HaouMjBviBKKTYZGV_9qtg/UserMedia";

export const getMediaChunk = async (cursor: string | undefined): Promise<{
    tweets: RawTweet[],
    cursor?: string
}> => {
    ...
    const response = await fetch(MEDIA_ENDPOINT + `?${queryString.stringify(query)}`, {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    if (!response.ok) {
        throw new Error(`user media endpoint returned ${response.status} (${response.statusText})`);
    }
