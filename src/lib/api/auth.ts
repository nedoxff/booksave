...
const getBearerToken = async (): Promise<string> => {
  ...
  if (!response.ok) {
    console.warn(
      "failed to receive a new bearer token. falling back to the default one",
    );
  ...
  if (body.token_type !== "bearer") {
    console.warn(
      `expected token_type to be 'bearer', received '${body.token_type}'. falling back to the default bearer token`,
    );
    return FALLBACK_TOKEN;
  }
...
export const getAuthorizationData =
  async (): Promise<TwitterAuthorization | null> => {
