...
const BOOKMARK_ENDPOINT: string =
  "https://x.com/i/api/graphql/L7vvM2UluPgWOW4GDvWyvw/Bookmarks";

export const getBookmarkChunk = async (
  ...
  const response = await fetch(
    BOOKMARK_ENDPOINT + `?${queryString.stringify(query)}`,
    {
      method: "GET",
      credentials: "include",
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `bookmark endpoint returned ${response.status} (${response.statusText})`,
    );
  }
  ...
  try {
    const instructions = (await response.json()).data.bookmark_timeline_v2
