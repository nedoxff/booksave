...
const LIKES_ENDPOINT: string =
  "https://x.com/i/api/graphql/px6_YxfWkXo0odY84iqqmw/Likes";

export const getLikesChunk = async (
  ...
  const response = await fetch(
    LIKES_ENDPOINT + `?${queryString.stringify(query)}`,
    {
      method: "GET",
      credentials: "include",
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `likes endpoint returned ${response.status} (${response.statusText})`,
    );
  }
