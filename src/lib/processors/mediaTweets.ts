...
import { getMediaChunk } from "$lib/api/media";
...
export const processMediaTweets = async () => {
    ...
    const stream = streamSaver.createWriteStream('media.zip');
    ...
    while (true) {
        const { tweets, cursor } = await getMediaChunk(currentCursor);
        ...
        await sendMessage('sendProcessingStats', { type: SourceOption.MEDIA_TWEETS, processed: processed, skipped: skipped });
    }
